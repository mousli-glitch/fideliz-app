...
- **P-15 ✅ TRANCHÉE le 19/08/2026 — oui, avant.** Migration `cartiz/082` écrite, non appliquée. ⚠️ La règle seule ne suffit pas : le versement doit porter l'ancre de rétention, sinon les compteurs repartent à zéro. Voir `p15-anonymisation-avant-versement.md`. *(recommandation d'origine : oui)* Sinon une donnée personnelle perd son échéance en changeant de table — et il faudra aussi purger `push_log.message` et `passes.message_push`, qui conservent le prénom rendu.
