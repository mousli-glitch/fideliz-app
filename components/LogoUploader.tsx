...
import { UploadCloud, Link as LinkIcon, X, Loader2, CheckCircle } from "lucide-react"
...
export default function LogoUploader({ currentUrl, onUrlChange }: LogoUploaderProps) {
  ...
  // --- VERSION CORRIGÉE : PNG + TRANSPARENCE ---
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          // ⚠️ EXPORT EN PNG (pour la transparence)
          canvas.toBlob((blob) => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1">
            {mode === 'UPLOAD' ? (
                <div className="relative border-2 border-dashed border-slate-300 rounded-xl bg-slate-50 hover:bg-blue-50 hover:border-blue-400 transition-all text-center p-6 group cursor-pointer h-32 flex flex-col items-center justify-center">
                    ...
                    <div className="flex flex-col items-center gap-2 text-slate-400 group-hover:text-blue-500">
                        {uploading ? (
                            <>
                                ...
                                <p className="text-xs font-bold text-slate-600">Optimisation PNG...</p>
                            </>
                        ) : (
                            <>
                                <UploadCloud size={24}/>
                                <div>
                                    <p className="text-sm font-bold text-slate-700">Glissez votre logo ici</p>
                                    <p className="text-[10px] text-slate-400">PNG Transparent supporté</p>
                                </div>
                            </>
                        )}
                    </div>
                ...
                <div className="relative">
                    ...
                    <input type="url" placeholder="https://..." value={currentUrl} onChange={(e) => onUrlChange(e.target.value)} className="w-full p-3 pl-10 border rounded-xl bg-white outline-none focus:ring-2 focus:ring-blue-500"/>
                </div>
            )}
        </div>
        ...
        {currentUrl && (
            <div className="flex flex-col items-center gap-2 shrink-0">
                <div className="relative w-24 h-24 border rounded-xl overflow-hidden bg-[url('https://transparenttextures.com/patterns/stardust.png')] bg-slate-100 shadow-sm flex items-center justify-center group">
                    ...
                    <button onClick={() => onUrlChange("")} className="absolute top-1 right-1 bg-white/90 rounded-full p-1 shadow hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"><X size={14}/></button>
                </div>
                <span className="text-[10px] font-bold text-green-600 flex items-center gap-1"><CheckCircle size={10}/> OK</span>
            </div>
        )}
      </div>
    </div>
