...
export function PublicGameClient({ game, prizes, restaurant }: Props) {
  ...
  const handleLandingClick = () => setStep('INSTRUCTIONS')
  ...
  const handleSpin = async () => {
    ...
    setTimeout(() => {
      ...
      confetti({ particleCount: 150, spread: 70, origin: { y: 0.6 }, colors: [restaurant.brand_color, '#FFD700', '#FFFFFF'] })
      setStep('FORM')
      setSpinning(false)
    }, 4000)
  }
    ...
    <div className="w-full max-w-sm mx-auto text-center relative">
      <AnimatePresence mode="wait">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-2xl mx-4 relative z-20">
                <div className="mb-4 flex justify-center">
                     {game.active_action === 'GOOGLE_REVIEW' 
                        ? <img src="https://upload.wikimedia.org/wikipedia/commons/c/c1/Google_%22G%22_logo.svg" className="w-16 h-16" alt="Google"/>
                ...
                <button onClick={handleLandingClick} className="w-full py-3 rounded-lg font-bold text-white shadow-lg transition-transform active:scale-95 bg-gradient-to-r from-blue-500 to-purple-600">
                    {game.active_action === 'GOOGLE_REVIEW' ? 'Noter' : "S'abonner"}
                </button>
            </div>
            <div className="mt-8 text-white/50 text-xs flex items-center justify-center gap-4 relative z-10"><span>Règlement</span><span>Contact Pro</span></div>
          </motion.div>
        )}
        ...
        {step === 'INSTRUCTIONS' && (
          <motion.div key="instructions" initial="hidden" animate="visible" exit="exit" variants={slideLeft} transition={{ duration: 0.3 }} className="absolute w-full left-0 top-0">
             ...
             <div className="bg-white rounded-3xl p-6 shadow-2xl mx-4 relative z-20">
                ...
                <button onClick={handleInstructionValidate} className="w-full py-3 rounded-lg font-bold text-white shadow-lg transition-transform active:scale-95 bg-blue-500 hover:bg-blue-600">
                    Valider
                </button>
            </div>
          </motion.div>
        ...
        {step === 'VERIFYING' && (
           <motion.div key="verifying" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="bg-[#EF5350] rounded-2xl p-8 shadow-2xl mx-4 text-white border-2 border-red-400 relative overflow-hidden mt-20">
                 ...
                 <div className="flex justify-center mb-4"><Loader2 className="w-8 h-8 animate-spin" /></div>
                 <p className="font-bold text-lg">Nous vérifions actuellement votre action...</p>
        ...
        {step === 'WHEEL' && (
          <motion.div key="wheel" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center gap-8 mt-10">
            ...
            <div className="relative w-72 h-72 md:w-80 md:h-80">
              ...
              <div ref={wheelRef} className="w-full h-full rounded-full border-4 border-white shadow-2xl overflow-hidden relative bg-white">
                {prizes.map((prize, i) => {
                  const rotate = (360 / prizes.length) * i; const skew = 90 - (360 / prizes.length);
                  return (<div key={prize.id} style={{ backgroundColor: prize.color, transform: `rotate(${rotate}deg) skewY(-${skew}deg)`, position: 'absolute', top: '0', right: '0', width: '50%', height: '50%', transformOrigin: '0% 100%' }} />)
                })}
              </div>
            ...
            <button onClick={handleSpin} disabled={spinning} className="bg-white text-slate-900 font-black text-xl py-4 px-12 rounded-full shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center gap-2">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md z-[100]">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-sm bg-[#EF5350] rounded-3xl p-6 shadow-2xl relative border-4 border-white/20">
                ...
                <div className="text-center mb-6">
                    ...
                    <p className="text-white font-bold text-lg mb-4">Vous avez gagné un "{winner.label}"</p>
                </div>
                <form onSubmit={handleFormSubmit} className="space-y-3">
                    <input required placeholder="Prénom" value={formData.firstName} onChange={(e) => setFormData({...formData, firstName: e.target.value})} className="w-full p-3 rounded-lg bg-white/20 border border-white/30 text-white placeholder-white/70 focus:outline-none focus:ring-2 focus:ring-white"/>
                    <input required type="email" placeholder="Email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full p-3 rounded-lg bg-white/20 border border-white/30 text-white placeholder-white/70 focus:outline-none focus:ring-2 focus:ring-white"/>
                    <input type="tel" placeholder="Numéro mobile" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="w-full p-3 rounded-lg bg-white/20 border border-white/30 text-white placeholder-white/70 focus:outline-none focus:ring-2 focus:ring-white"/>
                    <div className="flex items-start gap-3 mt-4">
                        <input type="checkbox" id="optin" checked={formData.optIn} onChange={(e) => setFormData({...formData, optIn: e.target.checked})} className="mt-1 w-5 h-5 rounded border-gray-300 text-red-600 focus:ring-red-500" />
                        <label htmlFor="optin" className="text-xs text-slate-900 font-medium leading-tight text-left">Si l'utilisateur ne souhaite pas recevoir d'offres commerciales, merci de cocher la case.</label>
                    </div>
                    <button type="submit" disabled={isSubmitting} className="w-full bg-black text-white font-bold py-4 rounded-full mt-4 hover:scale-105 transition-transform shadow-lg">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-sm mx-auto text-center mt-10 relative z-[60]">
            ...
            <div className="bg-[#E0E0E0] rounded-b-xl rounded-t-none p-6 shadow-2xl mx-4 relative pb-32">
                ...
                <div className="absolute bottom-4 left-4 right-4 space-y-2">
                    <button className="w-full bg-black text-white font-bold py-3 rounded-xl shadow-lg text-sm flex items-center justify-center gap-2">
                        <Check size={16}/> Enregistrer mon cadeau
                    </button>
