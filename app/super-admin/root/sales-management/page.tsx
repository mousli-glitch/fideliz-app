...
export default function SalesManagement() {
  ...
  // --- 2. CHARGEMENT OPTIMISÉ (Utilise l'action Service Role) ---
  async function loadSales() {
    setLoading(true)
    const result = await getSalesData()
    if (result.success) {
      // On garde exactement tes noms de variables (restaurants_count)
      setSalesUsers(result.data || [])
    } else {
      console.error("Erreur chargement via Action:", result.error)
      // Fallback sur ton ancienne méthode si l'action échoue (sécurité max)
      const { data: agents } = await supabase.from('profiles' as any).select('*').eq('role', 'sales')
      setSalesUsers(agents || [])
    }
    setLoading(false)
  }
  ...
  const toggleStatus = async (id: string, currentStatus: boolean, userEmail: string) => {
    ...
    if (error) {
      await logSystemError({ message: `Erreur statut pour ${userEmail}`, details: error })
      alert("Erreur lors du changement de statut")
      loadSales()
    } else {
      // LOG ROOT : On trace qui est activé/bloqué
  ...
  // --- 4. CRÉATION DU COMMERCIAL (Avec mouchard de recrutement) ---
  const handleCreateSales = async (e: React.FormEvent) => {
    ...
    if (authError) {
      await logSystemError({ message: `Échec recrutement (Auth): ${email}`, details: authError })
    ...
    if (authData.user) {
      const query: any = supabase.from('profiles' as any)
      const { error: profileError } = await query.update({ role: 'sales', is_active: true }).eq('id', authData.user.id)
  ...
  // --- 5. SUPPRESSION SÉCURISÉE (HÉRITAGE CONSERVÉ À 100%) ---
  const handleDelete = async (id: string, userEmail: string, count: number) => {
    ...
    if (currentUser && count > 0) {
      // LOGIQUE D'HÉRITAGE (ton code d'origine)
      const { error: updateError } = await (supabase.from('restaurants') as any)
        .update({ owner_id: currentUser.id })
        .eq('owner_id', id)

      if (updateError) {
        await logSystemError({ message: `ERREUR CRITIQUE TRANSFERT: ${userEmail}`, details: updateError })
    ...
    if (error) {
      ...
      // LOG ROOT : On confirme la suppression et le transfert
      await logSystemError({ 
        message: `Commercial supprimé : ${userEmail} (${count} restos récupérés)`, 
        level: 'warning' 
      })
      ...
      <div className="p-8 max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-12">
          ...
          <button 
            ...
            className="bg-blue-600 hover:bg-blue-500 px-6 py-4 rounded-3xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-blue-900/20 text-sm uppercase"
          >
          ...
          {loading ? (
            ...
            <div key={agent.id} className={`border p-6 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between transition-all gap-6 ${agent.is_active !== false ? 'bg-slate-800/40 border-slate-800' : 'bg-red-900/10 border-red-900/30'}`}>
              ...
              <div className="flex gap-4 items-center w-full md:w-auto justify-end border-t md:border-t-0 border-slate-800 pt-4 md:pt-0">
                <button 
                  onClick={() => toggleStatus(agent.id, agent.is_active, agent.email)}
                  className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-black text-[10px] uppercase transition-all shadow-lg ${
                    agent.is_active !== false ? 'bg-slate-800 text-slate-500 hover:bg-amber-500 hover:text-white' : 'bg-green-600 text-white hover:bg-green-500'
                  }`}
                >
                ...
                <button 
                  onClick={() => handleDelete(agent.id, agent.email, agent.restaurants_count)}
                  disabled={deleteLoading === agent.id}
                  className="bg-red-500/10 hover:bg-red-600 text-red-500 hover:text-white px-5 py-3 rounded-2xl transition-all shadow-lg disabled:opacity-50"
                >
        ...
        {showForm && (
          <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 p-10 rounded-[3rem] max-w-md w-full relative shadow-2xl">
              <button onClick={() => setShowForm(false)} className="absolute top-10 right-10 text-slate-500 hover:text-white transition-colors">
                <X size={28} />
              </button>
